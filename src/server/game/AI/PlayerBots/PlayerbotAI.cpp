...
m_FeastSpamTimer(0)
{
    ...
    // PlayerbotChatHandler *Tempare; // = teleport(m_bot);
    // Tempare->teleport(master);
    ...
    // PlayerbotChatHandler::teleport(m_bot);
    SetQuestNeedItems();
    m_bot->TeleportTo(*master);
...
uint32 PlayerbotAI::getSpellId(const char *args, bool master) const
{
    ...
    for(PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
    {
        ...
        const SpellEffectEntry *pSpellEffectInfo = sSpellEffectStore.LookupEntry(spellId);
        if(!pSpellInfo) continue;

        const std::string name = (const char*) pSpellInfo->SpellName[loc]; //>SpellName[loc];
        if(name.empty() || !Utf8FitTo(name, wnamepart)) continue;

        bool isExactMatch = (name.length() == wnamepart.length()) ? true : false;
        bool usesNoReagents = (pSpellInfo->SpellReagentsId <=  0) ? true : false;
...
uint32 PlayerbotAI::getSpellIdExact(const char *args, bool includePassive, bool master)
{
    ...
    for(PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
    {
        ...
        const SpellEntry *pSpellInfo = sSpellStore.LookupEntry(spellId);
        const SpellEffectEntry *pSpellEffectInfo = sSpellEffectStore.LookupEntry(spellId);
        const SpellReagentsEntry *pSpellReagentInfo = sSpellReagentsStore.LookupEntry(spellId);
        if(!pSpellInfo) continue;
        if(!pSpellEffectInfo) continue;
        if(pSpellEffectInfo->Effect == SPELL_EFFECT_LEARN_SPELL) continue; //This is a learn spell

        const std::string name = (const char*) pSpellInfo->SpellName[loc];
        if(name.empty()) continue;
        if(strcmp(name.c_str(),namepart.c_str())) continue;
        if(pSpellReagentInfo->Reagent[0] <=  0 && !foundMatchUsesNoReagents){ foundSpellId = spellId; foundMatchUsesNoReagents = true; }
...
//sLog->outError ("master_speed2 = %d", master_speed2);
        ...
        for(PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
        {
            ...
            if(!pSpellInfo)
                continue;
            const SpellInfo *spell; // = sSpellStore.LookupEntry(spellId);
            // pSpellInfo->
            if(spell->Effects[0].ApplyAuraName == SPELL_AURA_MOUNTED)
            {
                if((spell->Effects[1].ApplyAuraName == SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED)
                    && (spell->Effects[2].ApplyAuraName == SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED))
                {
                    
                    if((spell->Effects[1].BasePoints == master_speed1)
                        && (spell->Effects[2].BasePoints == master_speed2))
                    {
                        spellMount = spellId;
                        break;
                    }
                }
                else if((spell->Effects[2].ApplyAuraName == SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED)
                    && (spell->Effects[1].ApplyAuraName == SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED))
                {
                    if((spell->Effects[2].BasePoints == master_speed2)
                        && (spell->Effects[1].BasePoints == master_speed1))
                    {
                        spellMount = spellId;
                        break;
                    }
                }
                else if(spell->Effects[1].ApplyAuraName == SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED)
                {
                    if(spell->Effects[1].BasePoints == master_speed1 && master_speed2 <= 0) { spellMount = spellId; break; } //Has no secondary mount aura
...
bool PlayerbotAI::CanCast(const SpellEntry * pSpellInfo, Unit *target, bool castExistingAura, bool skipFriendlyCheck, bool skipEquipStanceCheck)
{
    ...
    if (!skipEquipStanceCheck)
    {
        ...
        const SpellShapeshiftEntry * pSpellShapeshiftInfo = sSpellShapeshiftStore.LookupEntry(spellId);
        if (pSpellShapeshiftInfo->Stances & formMask) { return true; }
        if (pSpellShapeshiftInfo->StancesNot && pSpellShapeshiftInfo->StancesNot & formMask) { return false; }
        //if (!m_bot->HasItemFitToSpellRequirements(pSpellInfo)) return false;
    }
    const SpellInfo *spell = GetSpellInfo(spellId);
    //Power Costs
    const SpellSchoolMask pSpellSchool = spell->GetSchoolMask();
    uint32 pPowerCost = spell->CalcPowerCost(m_bot, pSpellSchool);
    ...
    if (m_bot->GetCurrentSpellCastTime(pSpellInfo->Id) > 0 && m_bot->isMoving()) return false; //Cannot cast while moving

    //The target is immune or not?
    if (target->GetTypeId() != TYPEID_PLAYER) { if ( ((Creature*)target)->IsImmunedToSpell(spell)) { return false; } }
    else { if (target->IsImmunedToSpell(spell)) { return false; } }
    ...
    else if (spell->_IsPositiveSpell())
    {
    ...
    else if(text == "spells")
    {
        ...
        for(PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
        {
            ...
            if(itr->second->state == PLAYERSPELL_REMOVED)
                sLog->outString("PLAYERSPELL_REMOVED");
            if(itr->second->disabled)
                sLog->outString("PLAYERSPELL_DISABLED");
            if(SPELL_ATTR0_PASSIVE)
                sLog->outString("PLAYERSPELL_PASSIVE");
            

            const SpellInfo *const _pSpellInfo = GetSpellInfo(spellId);
            if(!_pSpellInfo)
                continue;
            ...
            std::string comp = ",";
            std::string temp = &_pSpellInfo->SpellName[loc];

            comp.append(&_pSpellInfo->SpellName[loc]);
            comp.append(",");
            ...
            sLog->outString("spellname -> [%s]", temp.c_str());

            alreadySeenList += _pSpellInfo->SpellName[loc];
            alreadySeenList += ",";

            if(_pSpellInfo->_IsPositiveSpell())
                posOut << " |cffffffff|Hspell:" << spellId << "|h[" << _pSpellInfo->SpellName[loc] << "]|h|r";
            else
                negOut << " |cffffffff|Hspell:" << spellId << "|h[" << _pSpellInfo->SpellName[loc] << "]|h|r";
        }
        ...
        text == "quests")
    {
        ...
        for (QuestStatusMap::iterator iter=m_bot->getQuestStatusMap().begin(); iter!=m_bot->getQuestStatusMap().end(); ++iter)
        {
            ...
            if (name.empty())
                continue;
            //out << qData->m_status << " ";
            if (qData->_status == QUEST_STATUS_NONE)
            {
                //out << "Quest " << " |cffffffff|Hquest:" << questId << "|h[" << name << "]|h|r" << " ";
                //out << " no status" << "\n";
                continue;
            }
            else if (qData->_status == QUEST_STATUS_COMPLETE)
            {
                out << "Quest " << " |cffffffff|Hquest:" << questId << "|h[" << name << "]|h|r" << " ";
                out << " complete" << "\n";
            }
            else if (qData->_status == QUEST_STATUS_UNAVAILABLE)
            {
                //out << "Quest " << " |cffffffff|Hquest:" << questId << "|h[" << name << "]|h|r" << " ";
                //out << " unavailable" << "\n";
                continue;
            }
            else if (qData->_status == QUEST_STATUS_INCOMPLETE)
            {
                out << "Quest " << " |cffffffff|Hquest:" << questId << "|h[" << name << "]|h|r" << " ";
                out << " incomplete" << "\n";
            }
            else if (qData->_status == QUEST_STATUS_AVAILABLE)
            {
                //out << "Quest " << " |cffffffff|Hquest:" << questId << "|h[" << name << "]|h|r" << " ";
                //out << " available" << "\n";
                continue;
            }
            else if (qData->_status == QUEST_STATUS_FAILED)
            {
                out << "Quest " << " |cffffffff|Hquest:" << questId << "|h[" << name << "]|h|r" << " ";
                out << " failed" << "\n";
            }
         ...
         for ( uint32 l=0; l<lootNum; l++ )
         {
             ...
             if ( m_needItemList[item->itemid]>0 )
             {
                 ...
                 if ( m_bot->CanStoreNewItem( NULL_BAG, NULL_SLOT, dest, item->itemid, item->count ) == EQUIP_ERR_OK )
                 {
                     ...
                     m_bot->GetAchievementMgr().UpdateAchievementCriteria( ACHIEVEMENT_CRITERIA_TYPE_LOOT_ITEM, m_bot, item->itemid, item->count );
                 }
             }
...
void PlayerbotAI::SetQuestNeedItems()
{
    ...
    for(QuestStatusMap::iterator iter=m_bot->getQuestStatusMap().begin(); iter!=m_bot->getQuestStatusMap().end(); ++iter)
    {
        ...
        if(qData->_status != QUEST_STATUS_INCOMPLETE) continue;

        //check for items we not have enough of
        for(uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; i++)
        {
            if(!qInfo->ReqItemCount[i] || (qInfo->ReqItemCount[i]-qData->_itemcount[i]) <= 0) continue;
            m_needItemList[qInfo->ReqItemId[i]] = (qInfo->ReqItemCount[i]-qData->_itemcount[i]);
        }
    }
