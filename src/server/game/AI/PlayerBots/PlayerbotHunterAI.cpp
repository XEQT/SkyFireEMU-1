...
{
    QueryResult result = CharacterDatabase.PQuery("SELECT * FROM character_pet WHERE owner = '%u' AND (slot = '%u' OR slot = '%u')",bot->GetGUIDLow(), PET_SLOT_DEFAULT, PET_SLOT_OTHER_PET);
    if(result)
        sLog->outString("Pet's are tested. and you have one");
    else
        sLog->outString("Pet's are tested. and you have none");
...
//    }
    ...
    if( pet && pet->isAlive() )
    {
        ...
        {
            if (GROWL) pet->GetCharmInfo()->SetSpellAutocast(GetSpellInfo(GROWL),true); //Autocast growl
            if (BAD_ATTITUDE) pet->GetCharmInfo()->SetSpellAutocast(GetSpellInfo(BAD_ATTITUDE),true);
            if (COWER) pet->GetCharmInfo()->SetSpellAutocast(GetSpellInfo(COWER),false);
            if (CastSpell(INTIMIDATION,m_bot)) { return; }
        }
        else
        {
            if (GROWL) pet->GetCharmInfo()->SetSpellAutocast(GetSpellInfo(GROWL),false); //Do not try to get aggro
            if (BAD_ATTITUDE) pet->GetCharmInfo()->SetSpellAutocast(GetSpellInfo(BAD_ATTITUDE),false);
            if (COWER) pet->GetCharmInfo()->SetSpellAutocast(GetSpellInfo(COWER),true); //Autocast cower
        }
    ...
    if ((isUnderAttack()  && pDist <= ATTACK_DISTANCE)) { m_role = BOT_ROLE_DPS_MELEE; }
...
void PlayerbotHunterAI::Pull()
{
    ...
    // check ammo
    uint32 ammo_id = 1; // GetPlayerBot()->GetUInt32Value(PLAYER_AMMO_ID);
    if (!ammo_id) {
