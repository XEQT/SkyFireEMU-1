...
bool ChatHandler::HandlePlayerbotCommand(const char *args)
{
   ...
   if (charnameStr.compare("all") != 0)
   {
       ...
       if (guid == 0 || (guid == _session->GetPlayer()->GetGUID()))
       {
    ...
    if (cmdStr.compare("add") == 0 || cmdStr.compare("login") == 0)
    {
        if (charnameStr.compare("all") == 0)
        {
            ...
            names=_session->GetPlayer()->GetCharacterList();
        ...
        {
            if(_session->GetPlayerBot(guid) != NULL)
            {
                PSendSysMessage("Bot already exists in world.");
                SetSentErrorMessage(true);
                return false;
            }
            _session->AddPlayerBot(guid);
        }
    ...
    else if (cmdStr.compare("remove") == 0 || cmdStr.compare("logout") == 0)
    {
        if (charnameStr.compare("all") == 0)
        {
            ...
            for (PlayerBotMap::const_iterator iter = _session->GetPlayerBotsBegin(); iter != _session->GetPlayerBotsEnd(); ++iter)
            {
        ...
        else
        {
            ...
            _session->LogoutPlayerBot(guid, true);
...
bool ChatHandler::HandlePlayerbotMainTankCommand(const char *args)
{
    ...
    uint64 pGuid = 0;
    char *charname ;
    Group *group = _session->GetPlayer()->GetGroup();
    ...
    if(!*args) {
        ...
        if (_session->GetPlayer()->GetGroup()->IsMember(guid)) {
