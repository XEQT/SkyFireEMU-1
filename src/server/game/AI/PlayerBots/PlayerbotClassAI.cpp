...
bool PlayerbotClassAI::listAuras(Unit *u)
{
    ...
    for(Unit::AuraMap::const_iterator itr = vAuras.begin(); itr!=vAuras.end(); ++itr)
    {
        ...
        const std::string name = (const char*) spellInfo->SpellName[loc];
...
Unit *PlayerbotClassAI::FindMainTankInRaid(Player *gPlayer)
{
    ...
    if (!unitList.empty()){
      for (std::list<Unit*>::iterator itr = unitList.begin() ; itr!=unitList.end();++itr) {
        ...
        {
            sLog->outString("Checking pets PCAI");
    ...
    sLog->outString("Checking pets PCAI Finished");
    mainTank = pPlayer;
    return pPlayer;
}
